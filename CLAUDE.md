...
**G1 also fires in `apply_plan_live` (GTT template layer)** — a synchronous G1 check at the top of `apply_plan_live` (`template_attach.py`) verifies every GTT leg quantity and the wing quantity against `plan.parent_lot_size` before any broker call. Returns `AttachResult.errors` immediately on failure; `place_gtt` / `place_order` are never reached. Sits upstream of `broker.translate_qty` (contracts→lots for MCX) and upstream of the adapter ceiling — so a sub-lot plan from a stale or wrong `parent_lot_size` fails cleanly at attach. `plan.parent_lot_size` is always resolved (never 0) by `apply_template_to_order` via `await get_lot_size()` before calling `apply_plan_live`.
