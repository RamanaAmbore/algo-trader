...
- **Derivatives candidates SSOT fix (commit 346a26dd)**: `candidatesDayPnl` in derivatives overlay now calls `livePositionDayPnl()` directly for each candidate position leg (same SSOT function as `positionsDayPnlStore`). The per-root day P&L sum from the overlay now correctly matches the Pulse positions TOTAL (NavStrip P1). `_dayPnlForLeg` helper is retained for `_legExpPnlDisplay`, flash updates, and per-root overlay aggregation.
