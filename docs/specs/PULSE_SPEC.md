...
**Underlying options picker sort** (Aug 2026, commit e6656b7e; refined Sept 2026):
- `underlyingOptionsForPicker` Tier 1 + Tier 2 sort by **three-tier hierarchy**:
...
- Operators see high-conviction positions first regardless of count. Example: CRUDEOIL with
  2 lots reliably beats COPPER with 1 lot even if COPPER has more position count
...
| 2026-09-08 | v1.13 Derivatives underlying picker qty-sort (reverts provisional-seed + localStorage from b5c1ea68): §17.2 updated — `underlyingOptionsForPicker` Tier 1/2 sort now uses three-tier hierarchy: `|qty| descending → posCount descending → alpha ascending`. CRUDEOIL with 2 lots reliably ranks above COPPER with 1 lot. Reverts `_provisionalSeed` flag and `localStorage['ramboq.derivatives.underlying']` persistence introduced in b5c1ea68 — `loadStrategy` fires before positions load causing `strategy = null` regression, and `_provisionalSeed` was not cleared on manual pick. |
