...
- `fetchChainExpiries` loop capped at 6 retries × 5s = 30s max hang (reduced from 40
  retries = 200s)
- Defense-in-depth guard when MCX instruments (e.g., CRUDEOIL) are missing from the
  chain expiry index due to name mismatch

**MCX instrument tradingsymbol-prefix normalization fix** (backend):
- Root cause: Kite's master instrument data uses spaced names in the `name` field
  (e.g., "CRUDE OIL") while the frontend sends unspaced tradingsymbols (e.g.,
  "CRUDEOIL")
- Backend now normalizes MCX underlying keys by **stripping trailing digits and
  spaces** from tradingsymbol via `re.sub(r'\d.*', '', tradingsymbol)` when
  building the expiry index (`_build_expiries_index`). This matches the frontend's
  virtual root derivation (`.replace(/\d.*$/, '')`)
- Both `_build_expiries_index` (instruments.py) and `_chain_quotes_build_sym_map`
  (options.py) use the same prefix-derivation logic for consistency
- Fast-path guard: `chain_quotes` endpoint now validates `und in _exp_index` before
  short-circuiting; key-miss falls through to slow-path scan instead of returning
  empty silently
- Result: MCX expiry chains load immediately for all variants (CRUDEOIL, CRUDEOILM,
  NATURALGAS, GOLD, GOLDM, etc.) without name-field dependency; frontend retry cap
...
- Fixes: "Fetching expiries…" placeholder UI clearing only after timeout or manual
  refresh
- Implemented as: `$effect` watches `chainUnderlying`; clears loading state on empty
  underlying
...
## 16. LTP Tick Flash — LTP Column Only

Only the LTP column flashes on tick across all Pulse grids. Chg% and other columns
remain static (no animation). LTP flashes are driven by SSE ticks with a tight
feedback loop.

**Flash scope — LTP-only (Sep 2026)**:
- LTP column: flashes on every SSE tick (4Hz) and broker poll update
- Chg% column: **no flash** (static text color only)
...
- **Magnitude-tiered flash variants removed**; uniform `tf-up`/`tf-down` CSS animation 
...
### 17.3 Options Payoff Chart Legend
...
**Chart behavior**:
- Y-axis shows profit/loss in currency units
- X-axis shows underlying spot price range (±range around current spot)
- Solid curve (P&L) updates every tick as spot price changes during the session
- Dashed curve (Exp P&L) updates when strategy or spot price changes materially
- Intersection of curves shows current spot price and live P&L / expiry P&L alignment

### 17.4 Candidate Leg Row — Pending Qty Chip
...
### 17.6 Expiry-Close Analysis — "Exp close" Badge Counts
...
### 17.7 Candidate Position Filtering — Closed Positions (qty=0) Now Always Included
...
- `--algo-green-badge` — badge background for positive indicators
...
**CSS rendering fix** (Sep 2026):
- `.cell-pos`, `.cell-neg`, `.cell-flat` rules now include `.ag-theme-algo` prefix
  (`MarketPulse.svelte` styles), ensuring correct ag-Grid cell specificity override.
  Positive/negative/flat values now render in correct colors across all Pulse grids
  (watchlist, positions, holdings, derivatives).
...
## 29. LTP Flash Simplification

Only LTP column flashes on tick across all Pulse grids. Chg%, Day P&L, and P&L
...
**Chg%, Day P&L %, and P&L columns** (static styling only):
- `change_pct`, `day_pnl_pct`, and `pnl` columns remain static (no animation)
- Text colors and backgrounds reflect directional values but do not flash on update
...
The LTP cell text color is now determined by reading the per-row `change_pct` field
(left grid) or `day_pnl_pct` field (right grid) directly, eliminating divergence
risk between LTP text color and day % text color. Helper function `_ltpCellClass()`
in `pulseColumns.js` falls back to `ltp vs close_price` comparison only when the
field is absent.

### Affected columns and surfaces

**MarketPulse grids**:
- Left grid: `ltp` (LTP) column flashes on updates (LTP-only)
  - Text-color class: `.ltp-tc-flash-up` / `.ltp-tc-flash-down`
  - Chg% column: static text color only (no flash)
