...
class OptionsController(Controller):
    ...
    # ── Margin optimizer (POST) ────────────────────────────────────────

    @post("/optimize-margin")
    async def optimize_margin(self, data: dict) -> dict:
        """Margin optimizer — scoped to (account, underlying). Given the
        operator's current legs for that combo, generates alternative
        structures (add_wing, convert_to_spread, roll_strike_otm) with
        similar EV/POP profile but lower margin requirement.

        Request body:
            account:            str       (required)
            underlying:         str       (required, e.g. "NIFTY")
            legs:               list[dict] (required, current open legs)
            spot:               float     (required, current underlying spot)
            ev_tolerance_pct:   float = 15.0
            max_loss_drift_pct: float = 25.0
            strategies:         list[str] = ["add_wing", "convert_to_spread",
                                              "roll_strike_otm"]
            force_refresh:      bool = false (bypass 30-min cache)

        Response: see margin_optimizer.OptimizeResult dict shape. The
        same response is served from cache for 30 minutes per
        (account, underlying, legs_fingerprint) so the operator's
        Refresh button is the only way to recompute within that
        window. Default cadence matches operator intent: optimization
        is a strategy-level decision, not a tick-level one.
        """
        from backend.api.algo.margin_optimizer import optimize, result_to_dict

        account     = str(data.get("account") or "").strip()
        underlying  = str(data.get("underlying") or "").strip().upper()
        legs        = list(data.get("legs") or [])
        spot        = float(data.get("spot") or 0.0)
        if not account or not underlying or not legs or spot <= 0:
            raise HTTPException(
                status_code=400,
        ...
        try:
            result = await optimize(
                ...
                legs_data=legs,
                spot=spot,
                ev_tolerance_pct=float(data.get("ev_tolerance_pct") or 15.0),
