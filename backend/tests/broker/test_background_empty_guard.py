...
class TestFetchPositionsEmptyGuard:
    ...
    def test_all_empty(self):
        ...
        with patch('backend.brokers.broker_apis.fetch_positions') as mock_fetch:
            ...
            # Empty-summary schema carries day_change_val + day_change_percentage so the
            # agent engine can read day_change_percentage (feeds pnl_rate_pct / day_pct).
            assert list(summary.columns) == ['account', 'pnl', 'day_change_val', 'day_change_percentage'], \
                f"summary should have ['account', 'pnl', 'day_change_val', 'day_change_percentage'], got {list(summary.columns)}"
            assert summary.empty, "summary should be empty when guard fires"
