...
def _run_override_stale_close_from_snapshot(
    ...
    # _fetch_snapshot_close_map's query now returns 6 columns (audit item
    # #4 — added `kind`/`qty` so the caller can gate a holdings-sourced
    # baseline). Pad legacy 4-tuple fixtures with (kind='positions', qty=None)
    # — a plain positions-kind baseline with no pro-ration context, matching
