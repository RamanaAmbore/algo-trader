...
function _ltpFlashClass(sym, getLtpFlashUp, getLtpFlashDown) {
  if (getLtpFlashUp?.().has(sym)) return 'tf-up';
  if (getLtpFlashDown?.().has(sym)) return 'tf-down';
  return null;
}
...
export function mkRightColDefs({
  ...
  return /** @type {any[]} */ ([
    ...
    { field: 'pnl_per_share', headerName: 'P&L/sh', colId: 'pnl_per_share',
      width: 78, minWidth: 60, maxWidth: 96,
      type: 'numericColumn', headerClass: numericHdr,
      cellClass: (p) => pnlCellClass(p, 'pnl_per_share'),
      valueFormatter: aggFmtGrid,
...
export function mkPosSummaryCols({ numericHdr, pnlCellClass, dirCellClass, aggFmtGrid, pctFmtGrid }) {
  return [
    ...
    { field: 'day_change_percentage', headerName: 'Chg %',   width: 60,
      type: 'numericColumn', headerClass: numericHdr,
