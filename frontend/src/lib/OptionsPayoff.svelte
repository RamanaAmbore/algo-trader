<script>
  ...
  let {
    ...
    // Sum of baseDayPnlForPosition (the baseline-diff Day P&L formula:
    // current_total_profit − prev_settlement_pnl) for the enabled
    // candidates — purely poll-driven (§1 redesign), no live-tick delta
    // term. Rendered as the DAY row in the stat overlay so the operator
    // can compare it against the PositionStrip's P∆ chip. Subset
    // relationship: when the basket covers EVERY open position in the
    // operator's book, DAY equals P∆ exactly. null / 0 → DAY row hides.
