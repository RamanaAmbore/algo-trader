<script>
  ...
  const sections = [
    ...
    },
    {
      ...
      tag: 'Trading',
      color: '#10b981',
      body: 'Underlying spot price, contract LTP, days-to-expiry, OI, volume, and bid-ask spread — all visible in the order modal before you place. ATM/ITM/OTM classification for options. Position cost and unrealized P&L shown for close orders.',
      bullets: [
        'Contract LTP displayed in the modal header next to the symbol name — live, updates every 2s',
        'Underlying spot price shown in header middle (for options/futures: the index or commodity root level)',
        'Days-to-expiry chip in symbol meta row for F&O contracts; amber when ≤3 days to expiry',
        'OI, Volume, bid-ask spread displayed in the market depth section',
        'Position entry price + unrealized P&L shown for CLOSE orders',
        'ATM/ITM/OTM classification shown for option orders',
        'Chase aggressiveness toggle (Low / Med / High) relocated to price-entry row for LIMIT order type',
        'Exchange-closed badge in header when market is closed',
        'Wing-leg warning when a template with MARKET wing is selected during closed hours',
      ],
      link: { href: '/orders', label: 'Open Orders' },
    },
    {
      title: 'Adaptive limit chasing',
      tag: 'Trading',
      color: '#ec4899',
      body: 'Three aggressiveness modes — Low (patient, pegs to opposite side), Med (midpoint, fills when spread moves halfway), High (crosses spread for urgent fills). Template exit rules auto-attach on fill.',
      bullets: [
        'Low mode: re-quote to bid (SELL) or ask (BUY) every tick, patient fill at market price',
        'Med mode: re-quote to midpoint, fill when spread tightens',
        'High mode: cross the spread if needed, urgent execution for time-sensitive positions',
        'Chase chain for LIMIT orders — cancel old + place new each tick, tracked in Order Activity',
        'Partial-fill handling: exit GTT (TP/SL) sized to actually-filled qty, not original ask',
        'Chase can be toggled per-ticket; persists across reloads when submitted',
      ],
      link: { href: '/orders', label: 'Open Orders' },
    },
    {
