...
\begin{{tikzpicture}}[remember picture,overlay]
  ...
  % Author name + RamboQuant role + Claude-Code tagline — below rule.
  \node[anchor=west] at ([xshift=0.9in,yshift=-1.5in]current page.west) {{%
...
% -- (3) Author bio — full width, recruiter-polish. Name in \huge with
% amber FRM chip; role band; bio explicitly ties FRM+CFA+industry to
% the RamboQuant build.
{{\color{{ramboqamber}}\sffamily\footnotesize\bfseries THE AUTHOR}}\\[2pt]
\noindent
...
{{\color{{ramboqslate}}\sffamily\small
\textbf{{30+ years}} across mainframe modernization and cloud-native financial platforms, including \textbf{{19 years at Fidelity Investments}}. Currently \textbf{{Principal System Analyst}} at Fidelity --- leading billing-platform modernization on AWS + Snowflake and distributed fee-calculation engines --- and concurrently the \textbf{{platform architect + quantitative developer}} building \textbf{{RamboQuant}} end-to-end (live at \textbf{{ramboq.com}}), applying a \textbf{{forward-deployment-engineer ethos}} (end-to-end ownership, tight feedback loops, ship-what-works) and leveraging \textbf{{Claude Code}} as a force multiplier for platform-scale solo output.
